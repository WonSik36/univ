...
public class Interpreter {
	...
	public ValueStore interp(AST ast, Dfs ds, Store st) {
		...
		if(ast instanceof Add) {
			...
			int res = Integer.parseInt(((NumV)vs1.getValue()).getStrNum()) 
					+ Integer.parseInt(((NumV)vs2.getValue()).getStrNum());
		...
		if(ast instanceof Id) {
			Id id = (Id)ast;
			return new VS(storeLookup(lookup(id.getId(),ds),st), st);				
		}
		
		if(ast instanceof Fun) {
			Fun fun = (Fun)ast;
			return new VS(new ClosureV(fun.getParam(), fun.getBody(), ds),st);
		}
