...
word	A candidate "word" that is then passed on to print_if_good 
		which prints the word if it meets the minimum requirements 
		for consonants.  The representation is again an array of characters
		terminated by a NULL ('\0').
							
dial	A 2-d array.  For each dial [i][j], i represents the actual number 
		on the key pad, and j stores a possible number that keypad could 
		represent.  So, for example, dial[2][3], could represent the 
		third letter that "1" on the keypad represents...
		or in this case the letter "c."  Similiarly, dial[2][2] == 'b', 
		dial [3][3] = f, etc.  Confused? Pick up your phone and take a look!  
		Some letters aren't mapped the way it should be.  
		Note, the letters "q" and "z" have been removed.  The numbers 0 and 1 
		have no assosciated letters and as such, the program simply returns 
		a "1" or a "0" in those circumstances.
