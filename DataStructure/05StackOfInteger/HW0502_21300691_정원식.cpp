 ...
 *
 *  Group work : I debugged runtime error. My partner made outline structure
 */
...
int push(stack *s, int key) {
	...
	if (isFull(s)) {                            
		if(s -> capacity == 0)                    //To prevent error when capacity become 0
			s->capacity = 1;            
		else                                      //expand capacity to double
			s->capacity*=2;
		
		memorySize = s->capacity * sizeof(element);          
		s->item = (element *)realloc(s->item, memorySize);   //memory rellocate by memorySize
		
		if(s == NULL){                                    //when fail memory allocating
...
int pop(stack *s) {
	...
	if ((s->N-1 <= (s->capacity / 4))  && (s->capacity != 1)) { //condition that shrinks value of capacity
		s->capacity/=2;
		memorySize = s->capacity * sizeof(element);
		s->item = (element *)realloc(s->item, memorySize);    //memory rellocate by memorySize
